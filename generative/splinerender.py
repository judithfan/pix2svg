    ...
    if n_nan < n * 3:  # has at least 1 root
        ...
        for i in range(3):
            ...
            if sum(ix_ge_not_nan_gt_0_le_1) > 0:
                ...
                if sum(ix_best) > 0:
                    dist_min[ix_ge_not_nan_gt_0_le_1][ix_best] = dist[ix_best]
                    t_min[ix_ge_not_nan_gt_0_le_1][ix_best] = root_ge_not_nan_gt_0_le_1[ix_best]
                    pos_min_x[ix_ge_not_nan_gt_0_le_1][ix_best] = pos_x[ix_best]
                    pos_min_y[ix_ge_not_nan_gt_0_le_1][ix_best] = pos_y[ix_best]
        
        ix_t_min_neq_maxint = t_min != sys.maxint
        ix_dist_min_lt_d0 = dist_min < d0
        ix_dist_min_lt_d2 = dist_min < d2
        try_endpoints += (ix_t_min_neq_maxint + ix_dist_min_lt_d0 + ix_dist_min_lt_d2)
        try_endpoints = 1 - torch.clamp(try_endpoints, 0, 1)
        # eq 1 if we want to try to find a point for it...
...
def solve_cubic_polynomial(a, b, c, d, eps=0.0000001):
    ...
    a_2, b_2, c_2, d_2 = a[1 - ix_3], b[1 - ix_3], c[1 - ix_3], d[1 - ix_3]
