...
class SketchOnlyDataset(Dataset):
    def __init__(self, layer='fc6', split='train', synthetic_labels=False, transform=None, random_seed=42):
        ...
        for annotation, choice in annotations:
            ...
            unrolled_dataset[annotation].append(one_hot(choice))
        ...
        dataset = []
        for path in unrolled_dataset.keys():
            if path in paths:
                labels = unrolled_dataset[path]
                n_labels = len(labels)
                dataset.append((path, np.sum(labels, axis=0) / float(n_labels)))
